...
3) `memories/` 文件夹当作规则与上下文来源。`implementation-plans/` 仅用于执行顺序参考，不可用于定义行为。
4) implementation-plans 仅用于定义执行步骤，不可作为规范来源，每次改动后，打开 progress.md 记录你做了什么供后续开发者参考，再把新的架构洞察添加到 architecture.md 中解释每个文件的作用；
5) LLM 在输出该项目文档的过程中，需要作为一个耐心的讲师，把专业知识用普通人都能懂的方式落地，尽量用中文。
-->
...
5. `docs/memories/archive-markdown-schema.md`：本项目专属，归档规范唯一源；

## 权威顺序（冲突处理）

1. `archive-markdown-schema.md`（最高优先级）
2. 其他 `memories/*.md`
